...
public class AiController implements InGameInterface {
	...
	@Override
	/**
	 * Starts the play with the given hand on the given position. The PlayState will
	 * be introduced later.
	 * 
	 * @author fkleinoe
	 * @param hand
	...
	public PlayState askToSetPlayState(PlayState playState) {
		...
		if (playState.getPlayMode() != PlayMode.NULL) {
			...
			while (!hasTrump[0] && j < this.bot.getHand().size()) {
				if (this.bot.getHand().get(j).getNumber() == Number.JACK || (playState.getPlayMode() == PlayMode.SUIT
	 ...
	 * currentTrick, playedCards, cardProbabilities, hasColour, existingTrumps
	 * hasTrump.
	 * 
	 * @author fkleinoe
	 * @param card
	...
	public void receivedNewCard(Card card, Player player) {
		...
		// Update hasColour
		if (this.currentTrick.get(0).getNumber() != Number.JACK && (this.playState.getPlayMode() == PlayMode.SUIT
		...
		if (this.existingTrumps != 0 && (this.currentTrick.get(0).getNumber() == Number.JACK
			...
			if (card.getNumber() != Number.JACK && card.getColour() != this.currentTrick.get(0).getColour()) {
				for (int i = 0; i < this.player.size(); i++) {
					if (this.player.get(i).getName().equals(Integer.toString(player.getId()))) {
	...
	@Override
	/**
	 * This method sets the PlayState, if the AI is not declarer. These parameters
	 * will be updated: partner, opponents, hasTrump, existingTrumps. Also the if
	 * the AI difficulty is hard, the AI is asked if it wants to declare kontra.
	 * 
	 * @author fkleinoe
	 * @param playState
	 */
	public void setPlaySettingsAfterAuction(PlayState playState) {
		...
		for (int i = 0; i < playState.getGroup().length; i++) {
			if (!playState.getGroup()[i].isDeclarer()
				...
				for (int j = 0; j < this.player.size(); j++) {
					if (this.player.get(j).getName().equals(Integer.toString(playState.getGroup()[i].getId()))) {
		...
		if (playState.getPlayMode() != PlayMode.NULL) {
			for (int i = 0; i < this.bot.getHand().size(); i++) {
				if (this.bot.getHand().get(i).getNumber() == Number.JACK || (playState.getPlayMode() == PlayMode.SUIT
