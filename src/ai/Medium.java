...
public class Medium {
	...
	public static int playCardGrand(AiController controller) {
		...
		if (controller.getBot().isDeclarer()) {
			...
			if (trick.size() == 0) {
				...
				if (trick.get(0).getNumber() == Number.JACK) {
					...
					if (controller.getHasColour()[3 - trick.get(0).getColour().ordinal()][0]) {
						...
						int number = 7;
						while (number >= 0
								&& controller.getCardProbabilities()[3 - trick.get(0).getColour().ordinal()][0] == 0) {
							number--;
						}
						if (number != -1) {
							for (int i = 0; i < cards.size(); i++) {
					...
					if (controller.getHasTrump()[0]) {
						...
						if (trickValue >= minTrickValue) {
							int number = 7 - Number.JACK.ordinal();
							int colour = 3;
							while (colour >= 0 && controller.getCardProbabilities()[colour * 8 + number][0] != 1) {
								colour--;
							}
							if (colour != 0) {
								for (int i = 0; i < cards.size(); i++) {
			...
			// 3rd card in trick
			if (trick.size() == 2) {
				Card declarerCard = controller.getPlayedCards()[controller.getPlayedCards().length - 1][declarer];
				int partner = (declarer == 1) ? 2 : 1;
				Card partnerCard = controller.getPlayedCards()[controller.getPlayedCards().length - 1][partner];
				...
				if (serveTrump) {
					...
					int colourValue = 3 - trick.get(0).getColour().ordinal();
					if (partnerCard.getNumber() == Number.JACK || (partnerCard.getColour() == trick.get(0).getColour()
							&& ((declarerCard.getColour() != trick.get(0).getColour()
									&& declarerCard.getNumber() == Number.JACK)
									|| (declarerCard.getColour() == trick.get(0).getColour() && declarerCard.getNumber()
											.ordinal() < partnerCard.getNumber().ordinal())))) {
